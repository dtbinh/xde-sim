...
class Robots:
    ...
    def xErrTS(self):
        ...
        aux = self.realTabl[:,2] - np.roll(self.plTabl[:,1], -1)
        return np.concatenate((aux[:-1], np.array([aux[-2]])))
    def yErrTS(self):
        ...
        # Rolling planned vector for accounting for simulation step time times velocity
        aux = self.realTabl[:,3] - np.roll(self.plTabl[:,2], -1)
        return np.concatenate((aux[:-1], np.array([aux[-2]])))
    def posErrTS(self):
        # return np.array([math.copysign(np.sqrt(ex**2+ey**2), np.cos(theta)*ey - np.sin(theta)*ex) for ex, ey, theta in zip(self.xErrTS(), self.yErrTS(), self.realTabl[:,4])])
        return np.array([np.sqrt(ex**2+ey**2) for ex, ey in zip(self.xErrTS(), self.yErrTS())])
    def thetaErrTS(self):
        aux =  np.array([wrapToPi(wrapToPi(a)-wrapToPi(b)) for a, b in zip(self.realTabl[:,4], np.roll(self.plTabl[:,3], -1))])
        return np.concatenate((aux[:-1], np.array([aux[-2]])))
    def realRadTS(self):
...
for sRSInfo in simulInfo[0:nbOfRobots]:
    ...
    figPose = plt.figure()
    axPose = figPose.gca()
    # figPose, axArrayPose = plt.subplots(2, sharex=True)

    # axArrayPose[0].grid()
    axPose.grid()

    # axArrayPose[0].set_ylim([-0.015, 0.015])
    # axPose.set_ylim([-0.1, 0.1])

    # axArrayPose[0].plot(simTime, sRSInfo.posErrTS(), 'b', label=r'$\|[x_{err}\ y_{err}]^T\|$')

    axPose.plot(simTime,  sRSInfo.planThetaTS(), 'b', label=r'$\theta_{ref}$')
    axPose.plot(simTime,  sRSInfo.realThetaTS(), 'r', label=r'$\theta$')

    # hand, lab = axArrayPose[0].get_legend_handles_labels()
    # axArrayPose[0].legend(hand, lab, ncol=1, prop={'size':10}, loc=2)
    hand, lab = axPose.get_legend_handles_labels()
    axPose.legend(hand, lab, ncol=1, prop={'size':10}, loc=2)

    figPose.savefig(direc+'build/out/Release/bin/images/theta'+ str(sRSInfo.id) +'.pdf', bbox_inches='tight', dpi=300)
    figPose.savefig(direc+'build/out/Release/bin/images/theta'+ str(sRSInfo.id) +'.png', bbox_inches='tight', dpi=300)
    ##########
