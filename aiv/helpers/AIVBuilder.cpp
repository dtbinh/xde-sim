...
namespace aiv {
		...
		)
	{
		...
		if ( pt.get<std::string>("root.mpmethod.<xmlattr>.type") == "receding horizon with termination" )
		{
			...
			pplanner->setOption("robotObstacleSafetyDist", pt.get<double>("root.mpmethod.robotobstaclesafetydist"));
			pplanner->setOption("waitForThread", pt.get<bool>("root.mpmethod.waitforthread"));
			// pplanner->setOption("numDerivativeEpsFBD", pt.get<double>("root.mpmethod.numderivativeepsfbd"));
			pplanner->setOption("numDerivativeEpsCD", pt.get<double>("root.mpmethod.numderivativeepscd"));
			pplanner->setOption("firstGuessMixCte", pt.get<double>("root.mpmethod.firstguessmixcte"));
