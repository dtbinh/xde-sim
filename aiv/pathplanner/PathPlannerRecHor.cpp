...
namespace aiv
{
	...
	PathPlannerRecHor::PathPlannerRecHor(std::string name, double updateTimeStep)
		...
		, _numDerivativeEpsFBD(1e-6)
		, _numDerivativeEpsCD(1e-6)
		, _radius(0.0)
		, _optIterCnter(0)
		, _timeOffset(Common::getRealTime<double>())
		, _spawnP0(false)
		, _firstGuessMixCte(1.5)
	{
	...
	void PathPlannerRecHor::setOption(const std::string& optionName, const double optionValue)
	{
		...
		else if (optionName == "numDerivativeEpsFBD")
		{
			_numDerivativeEpsFBD = optionValue;
		}
		else if (optionName == "numDerivativeEpsCD")
		{
			_numDerivativeEpsCD = optionValue;
		}
		else if (optionName == "firstGuessMixCte")
		{
			_firstGuessMixCte = optionValue;
		}
	}
			...
			const double myRadius) //const Displacementd & realPose, const Twistd &realVelocity)
	{
		...
		if(_detectedObstaclesMutex.try_lock())
		{
			_detectedObstacles = detectedObst;
			_detectedObstaclesMutex.unlock();
		}
		if(_knownRobotsMutex.try_lock())
		{
		...
		if(_radiusMutex.try_lock())
		{
			_radius = myRadius;
			_radiusMutex.unlock();
		}
		if(_comAIVsSetMutex.try_lock())
		{
			Common::KeysOnMapToVec(otherVehicles, _comAIVsSet);
			_comAIVsSetMutex.unlock();
		}
		
		// if(_currentInfoMutex.try_lock())
		// {
		// 	

		
		// 	_currentInfoMutex.unlock();
		// }
		...
		if (_planStage != DONE && _planStage != FINAL)
		{
			...
			{
				// _timeOffset = ;
				std::cout << "_______ " << std::string(name).substr(0,10) <<" ________ " << _executingPlanIdx << std::endl;
	...
	bool PathPlannerRecHor::_isAnyForbSpacInRobotsWayToTarget(MapObst forbiddenSpaces)
	{
		...
		for(std::map<std::string, Obstacle* >::iterator it=forbiddenSpaces.begin();it != forbiddenSpaces.end(); ++it )
		{
			...
			rad = it->second->getRad() + radius + _robotObstacleSafetyDist;

			forbSpacToRobot = _latestFlat - it->second->getCurrentPosition().getTranslation().block<2,1>(0,0);
			
			forbSpacToTarget = _targetedFlat - it->second->getCurrentPosition().getTranslation().block<2,1>(0,0);

			segmentNormTarg = (forbSpacToTarget - forbSpacToRobot).norm();

			determinantP1P2Targ = forbSpacToRobot(0,0)*forbSpacToTarget(1,0) - forbSpacToTarget(0,0)*forbSpacToRobot(1,0);
			...
			if (discriminantTarg >= 0.0) // itersection
			{
				...
				if (signed_r2o_proj_on_tdir <= radius)
				{
					continue;
				}
				else
				{
					std::cout << FG_B_L_CYAN << it->first << " is in the way to target" << RESET << std::endl;
					return true;
				}
			}
		}
	...
	Common::CArray3d PathPlannerRecHor::_estimateCollisionCenterAndRadius(const std::string& othRobName)
	{
		...
		double c = deltaP0x*deltaP0x + deltaP0y*deltaP0y - pow(radius + _knownRobots[othRobName]->getRad() + _interRobotSafetyDist, 2);
		...
		if (discriminant > 0)
		{
			...
			Common::CArray3d ret;
			ret[2] = _knownRobots[othRobName]->getRad() * std::min(1., radius*Common::finvsqrt(deltaP0x*deltaP0x + deltaP0x*deltaP0x));
			// ret[2] = 0.01;
	...
	bool PathPlannerRecHor::_isForbSpaceInRobotsWay(const std::string& fsName, MapObst forbiddenSpaces)
	{
		...
		double rad = forbiddenSpaces[fsName]->getRad() + radius + _robotObstacleSafetyDist;
		...
		else
		{
			...
			if (signed_r2o_proj_on_wdir <= radius*2)
			{
	...
	Common::CArray3d PathPlannerRecHor::_getAngularVariationAndDistForAvoidance(const std::string& fsName, MapObst forbiddenSpaces)
	{
		...
		std::cout << FG_B_L_YELLOW << fsName << ": " << theta1 << ", " << theta2 << RESET << std::endl;

		double refTheta = atan2(robotToForbSpac(1,0), robotToForbSpac(0,0)); // Principal arc tangent of y/x, in the interval [-pi,+pi] radians. http://www.cplusplus.com/reference/cmath/atan2/
		std::cout << FG_B_L_YELLOW << fsName << "::refTheta: " << refTheta << RESET << std::endl;

		double quad14RefTheta = (refTheta < M_PI_2 && refTheta >= -M_PI_2) ? refTheta : Common::wrapToPi(refTheta + M_PI);
		std::cout << FG_B_L_YELLOW << fsName << "::quad14RefTheta: " << quad14RefTheta << RESET << std::endl;

		if ((quad14RefTheta < theta1 && quad14RefTheta < theta2) || (quad14RefTheta > theta1 && quad14RefTheta > theta2))
		{
			std::cout << "angle to forbidden space is not between thetas" << std::endl;
		...
		std::cout << FG_B_L_YELLOW << fsName << "::changedThetas: " << theta1 << ", " << theta2 << RESET << std::endl;
	...
	void PathPlannerRecHor::_findNextWayPt()
	{
		...
		// 			// forbiddenSpaces[otherRobot->first+"_7"] = new ImaginaryForbiddenSpace(otherRobot->first+"_7", collInfo[2]/2., (FlatVector() << collInfo[0], collInfo[1]).finished()+
		// 			// 	(FlatVector() << otherDirec.x(), otherDirec.y()).finished()*6.5*_knownRobots[otherRobot->first]->getRad());
		// 			// forbiddenSpaces[otherRobot->first+"_8"] = new ImaginaryForbiddenSpace(otherRobot->first+"_8", collInfo[2]/2., (FlatVector() << collInfo[0], collInfo[1]).finished()+
		// 			// 	(FlatVector() << otherDirec.x(), otherDirec.y()).finished()*7.5*_knownRobots[otherRobot->first]->getRad());
		...
		if (forbiddenSpacesClusters.empty())
		{
			...
			// std::cout << "final theta: " << wayPtTheta << std::endl;
			return;
		}
		...
		for (VecVecStr::iterator clIt = forbiddenSpacesClusters.begin(); clIt != forbiddenSpacesClusters.end(); ++clIt)
		{
			...
			for (VecStr::iterator fsIt = clIt->begin(); fsIt != clIt->end(); ++fsIt)
			{
				// std::cout << FG_B_L_MAGENTA << clIt->front() << ": " << _avoidanceInfo[clIt->front()][0] << ", " << _avoidanceInfo[clIt->front()][1] << _avoidanceInfo[clIt->front()][2] << RESET << std::endl;
			...
			if (clIt->size() > 1)
			{
				...
				std::cout << this->name << "::sorted clockwise\033[1;36m";
				Common::printNestedContainer(*clIt);
				std::cout << "\033[0m" << std::endl;
				...
				std::cout << this->name << "::sorted anticlockwise\033[1;36m";
				Common::printNestedContainer(*clIt);
				std::cout << "\033[0m" << std::endl;
			...
			std::cout << "clIt size: " << clIt->size() << std::endl;
			std::cout << "clIt 1st: " << clIt->front() << std::endl;
			std::cout << "clIt 2nd: " << clIt->back() << std::endl;
			std::cout << "clIt 1st: " << FG_B_L_MAGENTA << clIt->front() << ": " << _avoidanceInfo[clIt->front()][0] << ", " << _avoidanceInfo[clIt->front()][1] << _avoidanceInfo[clIt->front()][2] << RESET << std::endl;
			std::cout << "clIt 2nd: " << FG_B_L_MAGENTA << clIt->back() << ": " << _avoidanceInfo[clIt->back()][0] << ", " << _avoidanceInfo[clIt->back()][1] << _avoidanceInfo[clIt->back()][2] << RESET << std::endl;
		...
		double eps = dTheta < 0.0 ? -0.01 : +0.01;
		// double eps = 0.0;
	...
	void PathPlannerRecHor::_plan()
	{
		...
		// RECIDING HORIZON STOP CONDITION [Validated do not revise]
		...
		// if (remainingDist < _lastStepMinDist + _compHorizon*_maxVelocity[FlatoutputMonocycle::linSpeedIdx])
		{
			...
			std::cout << "breakDist\n" << breakDist << std::endl;
			std::cout << "remainingDist\n" << remainingDist << std::endl;
			std::cout << "lastStepMinDist\n" << _lastStepMinDist << std::endl;
			std::cout << "compDist\n" << _compHorizon*_maxVelocity[FlatoutputMonocycle::linSpeedIdx] << std::endl;
		...
		//
		//_wayPt = _targetedFlat;
		std::cout << FG_B_L_BLUE << "WayPoint: " << RESET << _wayPt.transpose() << std::endl;
		...
		for (auto i=1; i<_optTrajectory.nParam(); ++i)
		{
			...
			if (_planLastPlan && prevDispl >= remainingDist - breakDist)
			{
				if (breakOn == false)
				{
					...
					accel = -1.*_maxAcceleration(FlatoutputMonocycle::linAccelIdx,0);
					breakOn = true;
				}
		...
		for (auto i=0; i < _optTrajectory.nParam(); ++i)
		{
			...
			p = (atan(magicNumber*double(i)/_optTrajectory.nParam()-magicNumber) + atan(magicNumber))/(atan(magicNumber));
			curve.col(i) = curveCurrDirec.col(i)*(1.0-p) + curveNewDirec.col(i)*p;
			// curve.col(i) = curveNewDirec.col(i);
			// curve.col(i) = curveCurrDirec.col(i);
		}
		...
		// curve = MatrixXd::Random(FlatoutputMonocycle::flatDim, _optTrajectory.nParam());
		...
		if (!gotInterrupted && _planStage != INIT)
		{
			_conflictEval();
	...
	bool PathPlannerRecHor::_solveOptPbl()
	{
		...
		else if (_optimizerType == "TESTINIT")
		{
			...
			// catch (boost::thread_interrupted&) { gotInterrupted = true; }
	...
	*/
	...
	// 		// 	x1[i] += h;
	// 		// }
	// 		for (int k = 0; x1[i] - x[i] == 0.0; ++k)
	// 			x1[i] = x[i] + pow(10.0, k)*h;
	...
	// 		{
	// 			grad[j*n + i] = (constrPos[j] - constrPre[j]) / (x1[i] - x[i]);
	// 			if (boost::math::isnan(grad[j*n + i]))
	// 			{
	// 				++nanCount;
	// 				std::cout << constrPos[j] << ", " << constrPre[j] << ", " << x1[i] - x[i] << ", " << std::endl;
	// 				for (int k=0; k < n; ++k)
	// 				{
	// 					std::cout << x[k] << ", ";
	// 				}
	...
	// 	for (int i = 0; i < int(n); ++i)
	// 	{
	// 		// h = x[i] < sqrtEps ? sqrtEps*sqrtEps : sqrtEps*x[i];
	// 		h = eps;
	...
	// 		(*eval)(f_at_x_minus_2h, n, x_minus_2h, this);
	// 		(*eval)(f_at_x_minus_h, n, x_minus_h, this);
	// 		(*eval)(f_at_x_plus_h, n, x_plus_h, this);
	// 		(*eval)(f_at_x_plus_2h, n, x_plus_2h, this);
	...
	// 		for (int j = 0; j < int(m)	; ++j)
	// 		{
	// 			grad[j*n + i] = (f_at_x_minus_2h[j] - 8.*f_at_x_minus_h[j] + 8.*f_at_x_plus_h[j] - f_at_x_plus_2h[j]) / (2.*(x[i] - x_minus_2h[i] + x[i] - x_minus_h[i] + x_plus_h[i] - x[i] + x_plus_2h[i] - x[i]));
	// 			if (boost::math::isnan(grad[j*n + i]))
	// 			{
	// 				++nanCount;
	// 				// std::cout << constrPos[j] << ", " << constrPre[j] << ", " << x1[i] - x[i] << ", " << std::endl;
	...
	// 				// }
	// 				// std::cout << std::endl << std::endl;
	// 				std::cout << FG_B_L_YELLOW << "NONSENSE" << RESET << std::endl;
	// 				grad[j*n + i] = 0.0;
	// 			}
	// 		}
	// 		x_minus_2h[i] = x[i];
	...
	// 		for (int j = 0; j < int(m)	; ++j)
	// 		{
	// 			grad[j*n + i] = (-3.*f_at_x[j] + 4.*f_at_x_plus_h[j] - f_at_x_plus_2h[j]) / (2.*h);
	// 			if (boost::math::isnan(grad[j*n + i]))
	// 			{
	// 				++nanCount;
	// 				// std::cout << constrPos[j] << ", " << constrPre[j] << ", " << x1[i] - x[i] << ", " << std::endl;
	...
	// forward and backward-difference second-order-accurate three-point + five-point formulae 
	void PathPlannerRecHor::computeNumGrad(unsigned m, unsigned n, const double* x, double* grad, void (*eval)(double*, unsigned, volatile double*, PathPlannerRecHor*))
	{
		// const double sqrtEps = sqrt(std::numeric_limits< double >::epsilon())*_numDerivativeEpsFBD;
		// const double eps = 1e-6;
		// double h = _numDerivativeEpsFBD;
		double h;
		...
		const double sqrtEps = _numDerivativeEpsCD;
	...
	// 	int firstIndex = 0;
	// 	if (_planLastPlan == true)
	// 	{
	// 		// const double sqrtEps = sqrt(std::numeric_limits< double >::epsilon())*_numDerivativeEpsFBD;
	// 		const double sqrtEps = _numDerivativeEpsCD;
	// 		//compute for i=0 all m js with the five-points formula
	// 		h = x[0] < sqrtEps ? sqrtEps*sqrtEps : sqrtEps*x[0];
			
	// 		x_minus_2h[0] = x[0] - 2.*h;
	// 		x_minus_h[0] = x[0] - h;
	// 		x_plus_h[0] = x[0] + h;
	// 		x_plus_2h[0] = x[0] + 2.*h;
	...
	// 		for (int j = 0; j < int(m)	; ++j)
	// 		{
	// 			grad[j*n] = (f_at_x_minus_2h[j] - 8.*f_at_x_minus_h[j] + 8.*f_at_x_plus_h[j] - f_at_x_plus_2h[j]) / (12.*h);
	// 			if (boost::math::isnan(grad[j*n]))
	// 			{
	// 				++nanCount;
	// 				// std::cout << constrPos[j] << ", " << constrPre[j] << ", " << x1[i] - x[i] << ", " << std::endl;
	// 				// for (int k=0; k < n; ++k)
	...
	// 	for (int i = firstIndex; i < int(n); ++i)
	// 	{
	...
	// 		if (i == firstIndex)
	// 		{
	// 			const double sqrtEps = _numDerivativeEpsFBD;
	// 			h = x[i] < sqrtEps ? sqrtEps*sqrtEps : sqrtEps*x[i];
	...
	// 			for (int j = 0; j < int(m)	; ++j)
	// 			{
	// 				grad[j*n + i] = (3.*f_at_x[j] - 4.*f_at_x_minus_h[j] + f_at_x_minus_2h[j]) / (2.*h);
	// 				if (boost::math::isnan(grad[j*n + i]))
	// 				{
	// 					++nanCount;
	// 					// std::cout << constrPos[j] << ", " << constrPre[j] << ", " << x1[i] - x[i] << ", " << std::endl;
	...
	double  PathPlannerRecHor::objectFunc(unsigned n, const double *x, double *grad, void *data)
	{
		...
		// TESTING FOR INTERRUPTIONS
		// try { boost::this_thread::sleep(boost::posix_time::milliseconds(2000)); }
