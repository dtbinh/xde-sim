...
def add_cmdline_options(parser):
	...
	parser.add_option('-x', '--xtol', dest='xtol', default=1E-9, action='store', type='float', help='optimization tolerance for parameters')
	...
	parser.add_option("-R", "--numderivfactor", action="store", type="float", dest="numdDerivFactor", default=100)
...
obstsInfo = randObsts(options.nObsts, Boundary([-6.0, 6.0], [-6.0, 6.0]), robotsInfo, 0.3, 1.1)
...
ET.SubElement(mpmeth, 'numderivativefactor').text = str(options.numdDerivFactor)
