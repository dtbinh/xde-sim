...
def add_cmdline_options(parser):
	...
	parser.add_option('-s', '--simspeed', dest='simSpeed', default=100.0, action='store', type='float', help='simulation time / wallclock time (if simspeed=1, 1 simulated second takes 1 real second). The max speed will be limited by the PC specs')
	...
	parser.add_option('-b', '--robots', dest='nRobots', default=2, action='store', type='int', help='number of robots')

	parser.add_option('-o', '--obstacles', dest='nObsts', default=3, action='store', type='int', help='number of obstacles')
	...
	parser.add_option('-l', '--lastsecmindist', dest='terminationDist', default=0.5, action='store', type='float',help='minimal distance left for completing the last section of the planning (in meters)')

	parser.add_option('-i', '--irdist', dest='interRobDist', default=0.1, action='store', type='float', help='minimal allowed distance between two robots (in meters)')

	parser.add_option('-O', '--rodist', dest='robotObstDist', default=0.1, action='store', type='float', help='minimal allowed distance between a robot and a obstacle (in meters)')
	...
	parser.add_option("-U", "--ctrlmaxout1", action="store", type="float", dest="cU1", default=2.0)

	parser.add_option("-u", "--ctrlmaxout2", action="store", type="float", dest="cU2", default=18.0)

	parser.add_option("-f", "--predictionhor", action="store", type="float", dest="predicHor", default=0.6)
...
def randRobots(no, boundary, rad):
		...
		if len(robots) > 0:
			for robot in robots:
				if (c[0]-robot[0][0])**2 + (c[1]-robot[0][1])**2 < (r+robot[1]+2.0)**2 or \
					(cf[0]-robot[3][0])**2 + (cf[1]-robot[3][1])**2 < (r+robot[1]+2.0)**2 :
					return False
	...
	x_range = _frange(boundary.x_min+rad, boundary.x_max-rad, resol)
	y_range = _frange(boundary.y_min+rad, boundary.y_max-rad, resol)
	t_range = _frange(-np.pi, np.pi - resol, resol)
	dist_range = _frange(4.5, 12.0, resol)
...
# robotsInfo = randRobots(options.nRobots, Boundary([-10.0, 10.0], [-10.0, 10.0]), 2.5*0.19664)
...
obstsInfo = randObsts(options.nObsts, Boundary([-8.0, 8.0], [-8.0, 8.0]), robotsInfo, 0.3, 1.0)
...
ET.SubElement(mpmeth, 'numderivativefactor').text = str(1.0)
...
ET.SubElement(optimizer , 'ftolerance').text = str(options.xtol)
